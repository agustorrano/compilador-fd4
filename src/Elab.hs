...
elab' env (SLet p False (v,vty) xs def body) =  
  Let p v vty (elab' env (SLam p xs def)) (close v (elab' (v:env) body))
-- Let Rec
elab' env (SLet p True l1@(v,vty) xs def body) =  
  Let p v vty (elab' env (SFix p l1 xs def)) (close v (elab' (v:env) body))
