...
tc (V p (Free n)) bs = case lookup n bs of
                           Nothing -> failPosFD4 p $ "Variable Free no declarada "++ppName n
                           Just ty -> return (V (p,ty) (Free n)) 
tc (V p (Global n)) bs = case lookup n bs of
                           Nothing -> failPosFD4 p $ "Variable Global no declarada "++ppName n
...
tcDecl :: MonadFD4 m  => Ty -> Decl Term -> m (Decl TTerm)
tcDecl ty (Decl p n t) = do
